...
namespace BNG {
    ...
    public class PluginsEditor : EditorWindow {
        ...
        void OnGUI() {
            ...
            // Warning about Fixed Timnstep being set to default 
            if (Time.fixedDeltaTime >= 0.02f) {
                GUILayout.Label("Project Settings Issue : ", EditorStyles.boldLabel);

                EditorGUILayout.HelpBox("WARNING! Your Fixed Timestep is set to " + Time.fixedDeltaTime + " which can result in choppy physics. Go to Project Settings -> Time and set to your targeted vr headset's display refresh rate (ie 1/72 or 1/90).", MessageType.Error);
                EditorGUILayout.Separator();
            }
